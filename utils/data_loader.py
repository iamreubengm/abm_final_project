...
class DataLoader:
    ...
    def import_investment_portfolio(self, file_path: str) -> Dict:
        """
        Import investment portfolio data from CSV.
        
        Args:
            file_path: Path to the CSV file
            
        Returns:
            Dictionary with processed portfolio data
        """
        try:
            # Read CSV
            df = pd.read_csv(file_path)
            ...
            for _, row in df.iterrows():
                holding = {
                    ...
                    "asset_class": str(row.get("AssetClass", row.get("asset_class", ""))),
            ...
            for holding in holdings:
                ...
                asset_classes[asset_class] += holding["value"]
            
            # Calculate asset allocation percentages
            asset_allocation = {}
            for asset_class, value in asset_classes.items():
                asset_allocation[asset_class] = (value / total_value) * 100 if total_value > 0 else 0
            ...
            print(f"Error importing investment portfolio: {e}")
            return {"holdings": [], "total_value": 0, "asset_allocation": {}, "count": 0}
    
    def generate_sample_user_data(self) -> Dict:
        """
        Generate sample user data for testing and demonstration.
        
        Returns:
            Dictionary containing sample user financial data
        """
        return {
            "personal": {
                "name": "Sample User",
            ...
            "debts": {
                "credit_cards": [
                    {"name": "Chase Sapphire", "balance": 3500, "interest_rate": 18.99, "minimum_payment": 100},
                    {"name": "Amex Gold", "balance": 2500, "interest_rate": 15.99, "minimum_payment": 75}
                ],
                "student_loans": [
                    {"name": "Federal Student Loan", "balance": 25000, "interest_rate": 4.5, "minimum_payment": 300}
                ],
                "mortgage": [
                    {"name": "Primary Residence", "balance": 320000, "interest_rate": 3.25, "minimum_payment": 1800}
                ],
                "auto_loans": [
                    {"name": "Car Loan", "balance": 15000, "interest_rate": 4.0, "minimum_payment": 350}
                ],
                "personal_loans": [],
                "other_loans": []
            },
            "investments": {
                "retirement_accounts": [
                    {"name": "401(k)", "balance": 120000, "contribution_rate": 10, "asset_allocation": {"stocks": 80, "bonds": 20}},
                    {"name": "Roth IRA", "balance": 45000, "contribution_rate": 0, "asset_allocation": {"stocks": 90, "bonds": 10}}
                ],
                "brokerage_accounts": [
                    {"name": "Vanguard", "balance": 65000, "asset_allocation": {"stocks": 75, "bonds": 15, "cash": 10}}
                ],
                "real_estate": [
                    {"name": "Primary Residence", "estimated_value": 550000, "mortgage_balance": 320000}
                ],
                "other_investments": []
            },
            "savings": {
                ...
                "savings_accounts": [
                    {"name": "High-Yield Savings", "balance": 25000, "interest_rate": 1.5, "purpose": "General Savings"}
                ],
                "savings_goals": [
                    {"name": "Vacation", "target": 5000, "current": 2500, "deadline": "2025-08-01"},
                    {"name": "Home Renovation", "target": 20000, "current": 5000, "deadline": "2026-06-01"}
                ]
            },
            "tax_info": {
                "income_tax_rate": 24,
            ...
            "profile": {
                "risk_tolerance": "moderate",
                "financial_goals": [
                    {"name": "Retirement", "priority": "high", "timeline": "long"},
                    {"name": "College Fund", "priority": "medium", "timeline": "medium"},
                    {"name": "Home Renovation", "priority": "medium", "timeline": "short"}
                ],
                "time_horizon": "medium"
            },
            "monthly_cashflow": {
                "total_income": 8917,  # (85000 + 15000 + 5000 + 2000) / 12
                "total_expenses": 5800,  # Sum of all expenses
                "surplus_deficit": 3117   # Income - Expenses
            },
            "credit_score": 740,
            "last_updated": datetime.now().isoformat()
        }